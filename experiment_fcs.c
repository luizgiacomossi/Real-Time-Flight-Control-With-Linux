...
/*
// Scheduling attributes structure for SCHED_DEADLINE
struct sched_attr {
    uint32_t size;
...
int main(int argc, char *argv[]) {
    ...
    char sched_policy[10] = "deadline";     // Default scheduler
    ...
    for (int k = 0; k < num_jobs; k++) {
        ...
        while (ts_next.tv_nsec >= 1000000000L) { // Handle nanosecond overflow
            ts_next.tv_sec++;
    ...
    for (int i = 0; i < num_jobs; i++) {
        if (resptimes_us[i] < min_resp) min_resp = resptimes_us[i];
        if (resptimes_us[i] > max_resp) max_resp = resptimes_us[i];
