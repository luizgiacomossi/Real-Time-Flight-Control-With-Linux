...
subplot(1, 3, 1);
plot_box_subplot(all_data(other_stress_indices), labels_other, '(a)Other Scheduler – Stress', [0.8 0.2 0.2], 0);
%set(gca, 'YScale', 'log')

% Subplot 1: Rest - No Stress
subplot(1, 3, 2);
plot_box_subplot(all_data(rest_no_stress_indices), labels_rest, '(b)RT Schedulers – No Stress', [0.2 0.6 0.2] , 0);
%set(gca, 'YScale', 'log')

% Subplot 2: Rest - Stress
subplot(1, 3, 3);
plot_box_subplot(all_data(rest_stress_indices), labels_rest, '(c)RT Schedulers – Stress', [0.8 0.2 0.2], 1);
%set(gca, 'YScale', 'log')
...
function plot_box_subplot(data_cells, labels, title_str, color, linear)
    ...
    if linear
        ...
        yscale log
        yticks([2 5 10 25 50 100 150 200 500 1000 2000 4000 10000])
    ...
    low = prctile(values, 0);
    high = prctile(values, 100);
    ylim([low high])
