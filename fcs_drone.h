...
static inline void positionController(const Vector3 *current_pos, const Vector3 *current_vel,
  ...
  output_force->x =
      gains->Kp_pos * pos_error.x - gains->Kd_pos * current_vel->x;
  output_force->y =
      gains->Kp_pos * pos_error.y - gains->Kd_pos * current_vel->y;
  output_force->z = gains->Kp_pos * pos_error.z -
                    gains->Kd_pos * current_vel->z +
                    9.81f; // Add gravity compensation
}
...
static inline void controlAllocator1(const Vector3 *force, float reference_yaw,
  ...
  *total_thrust =
  ...
  if (*total_thrust < 0.1f) {
    ...
    desired_attitude->pitch =
        -asinf(force->x / (*total_thrust * cosf(desired_attitude->roll)));
  }
...
static inline void attitudeController(const Attitude *current_attitude,
  ...
  output_torque->x =
      gains->Kp_att * roll_error - gains->Kd_att * current_angular->p;
  output_torque->y =
      gains->Kp_att * pitch_error - gains->Kd_att * current_angular->q;

  // PD controller for yaw
  output_torque->z =
      gains->Kp_yaw * yaw_error - gains->Kd_yaw * current_angular->r;
}
...
static inline void controlAllocator2(float thrust, const Vector3 *torque,
  ...
  float t1 = thrust / 4.0f - torque->y / (4.0f * l) + torque->x / (4.0f * l) -
             torque->z / (4.0f * k_m);
  float t2 = thrust / 4.0f + torque->y / (4.0f * l) - torque->x / (4.0f * l) -
             torque->z / (4.0f * k_m);
  float t3 = thrust / 4.0f - torque->y / (4.0f * l) - torque->x / (4.0f * l) +
             torque->z / (4.0f * k_m);
  float t4 = thrust / 4.0f + torque->y / (4.0f * l) + torque->x / (4.0f * l) +
             torque->z / (4.0f * k_m);
...
static inline void updateFlightControl(const QuadcopterState *current_state,
  ...
  positionController(&current_state->position, &current_state->velocity,
                     &reference->position, gains, &control_outputs->force);

  // 2. Control Allocator 1
  controlAllocator1(&control_outputs->force, reference->yaw,
                    &control_outputs->desired_att,
                    &control_outputs->total_thrust);

  // 3. Attitude Controller
  attitudeController(&current_state->attitude, &current_state->angular,
                     &control_outputs->desired_att, gains,
                     &control_outputs->torque);

  // 4. Control Allocator 2
  controlAllocator2(control_outputs->total_thrust, &control_outputs->torque,
                    motor_distance, thrust_coefficient, torque_coefficient,
                    motor_commands);
}
